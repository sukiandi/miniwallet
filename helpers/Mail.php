...
class Mail
{
     ...
     * @param \mero\Monolog $logger
     * @return bool whether the mail was sent successfully
     */
    public static function toUser($user, $view, $params = [], $logger = null)
    {
        $params['user'] = $user;
        return self::to($user->email, 'user/' . $view, 'layouts/user', $params, $logger);
    }
     ...
     * @return bool whether the mail was sent successfully
     */
    public static function to($recipient, $view, $layout, $params = [], $logger = null)
    {
        ...
        try {
            ...
            if ($message->send()) {
                if ($logger)
                    $logger->log('info', '-- Signup Email successfully sent to ' . $recipientString);
                Yii::info("Sent $view to $recipientString", __METHOD__);
                return true;
            } else {
                if ($logger)
                    $logger->log('error', '-- Signup Email failed to send to ' . $recipientString);
            ...
            if ($logger)
                $logger->log('error', '-- Swift_SwiftException was caught.', ['exception' => $e, 'trace' => $trace]);
